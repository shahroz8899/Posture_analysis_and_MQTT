...
import util
import os
from mqtt_receiver import start_mqtt_receiver
from mqtt_sender import send_posture_message  # Import the function
import threading
...
def checkPosition(all_peaks):
    try:
        ...
        if (all_peaks[16]):
            ...
            a = all_peaks[17][0][0:2] #Left Earunchback
...
mqtt_thread = threading.Thread(target=start_mqtt_receiver)
mqtt_thread.start()
...
while True:
    ...
    if os.path.exists('./received_images/img.jpg'):
        params, model_params = config_reader()
        canvas, position = process('./received_images/img.jpg', params, model_params)
     ...
     # Send posture message in a separate thread
        message_thread = threading.Thread(target=send_posture_message, args=(position,))
        message_thread.start()
			
    else:
            print("Error: No picture in the folder.")
